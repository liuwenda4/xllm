...
DEFINE_bool(enable_aclshmem_moe,
            false,
            "Enable the experimental ACLSHMEM TileLang MoE communication "
            "backend when an exact runtime specialization is available.");
...
void KernelConfig::from_flags() {
  ...
  XLLM_CONFIG_ASSIGN_FROM_FLAG(enable_aclshmem_moe);
...
void KernelConfig::from_json(const JsonReader& json) {
  ...
  XLLM_CONFIG_ASSIGN_FROM_JSON(enable_aclshmem_moe);
...
void KernelConfig::append_config_json(
  ...
  APPEND_CONFIG_JSON_VALUE_IF_NOT_DEFAULT(
      config_json, default_config, enable_aclshmem_moe);
  APPEND_CONFIG_JSON_VALUE_IF_NOT_DEFAULT(
