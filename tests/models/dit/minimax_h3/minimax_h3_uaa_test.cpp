...
TEST(MiniMaxH3UAAHcclTest, ForwardAttentionInverseAndProductionShape) {
  ...
  for (const torch::Tensor& value : local_qkv) {
    u_qkv.emplace_back(
        minimax_h3_uaa_launch_forward(value, u_group.get()).finish());
  }
  ASSERT_EQ(rank_device.synchronize_default_stream(), 0);
  ...
  for (int64_t index = 0; index < 3; ++index) {
    ...
    passed = passed && torch::equal(u_qkv[index].to(torch::kCPU), expected);
    passed =
        passed && minimax_h3_uaa_dummy_heads_are_zero(u_qkv[index], real_heads);
  }

  const torch::Tensor restored =
      minimax_h3_uaa_launch_inverse(u_qkv[0], u_group.get()).finish();
  ASSERT_EQ(rank_device.synchronize_default_stream(), 0);
  passed = passed &&
  ...
  if (environment->production) {
    ...
    const torch::Tensor production_u =
        minimax_h3_uaa_launch_forward(production_input, u_group.get()).finish();
    ASSERT_EQ(rank_device.synchronize_default_stream(), 0);
    ...
    const torch::Tensor production_restored =
        minimax_h3_uaa_launch_inverse(production_u, u_group.get()).finish();
    ASSERT_EQ(rank_device.synchronize_default_stream(), 0);
