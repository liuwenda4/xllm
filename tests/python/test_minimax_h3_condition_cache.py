...
def _official_cache_args(tmp_path: Path) -> dict[str, Any]:
    ...
    _save_archive(source_archive, _hidden(), torch.tensor([0, 1, 2], dtype=torch.int64))
...
def test_cache_key_is_canonical_and_preserves_reference_order() -> None:
    common = {
        "backend": "official_hf",
        "source_identity": [{"role": "official_reference", "sha256": "a" * 64}],
...
def test_official_extraction_squeezes_batch_and_makes_bfloat16_contiguous() -> None:
    source = {
        ...
        "h3_token_tags": torch.tensor([0, 1, 2], dtype=torch.int64),
    }
    ...
    assert tensors["text_token_tags"].tolist() == [0, 1, 2]
...
def test_native_extraction_uses_only_explicit_official_tags() -> None:
    ...
    official = {"h3_token_tags": torch.tensor([0, 1, 2], dtype=torch.int64)}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_hidden(), torch.tensor([0, 1, 3]), "subset of"),
        (_hidden(), torch.tensor([0, 1, 2], dtype=torch.int32), "must be int64"),
    ],
)
...
def test_official_cache_manifest_and_condition_provenance(tmp_path: Path) -> None:
    ...
    assert manifest["backend_approval_status"] == "approved correctness fallback"
...
def test_native_cache_records_experimental_provenance(tmp_path: Path) -> None:
    ...
    _save_archive(official_archive, torch.zeros(1), torch.tensor([2, 1, 0], dtype=torch.int64))
    ...
    assert load_file(str(cache_dir / "condition.safetensors"))["text_token_tags"].tolist() == [2, 1, 0]
...
def test_cli_defaults_to_official_and_prints_cache_directory(
    ...
    result = condition_cache.main(["--source-archive", str(source_archive), "--cache-root", str(cache_root)])
